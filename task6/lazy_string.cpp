...
lazy_string::lazy_string():
        ...
        begin(0),
        length(0){}
lazy_string lazy_string::substr(size_t begin, size_t length) const{
...
lazy_string::my_char lazy_string::at(size_t pos) {
    if (pos >= length) {
        throw std::out_of_range("lazy_string");
    }
...
lazy_string::my_char lazy_string::operator[](size_t pos){
    return at(pos);
}
...
size_t lazy_string::get_length() {
    return length;
}

lazy_string::my_char::my_char(lazy_string *const ls, size_t pos): ls(ls), pos(pos){}
